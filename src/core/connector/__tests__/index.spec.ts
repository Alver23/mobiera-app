// Shared
import { internalAxiosInstance } from '../../axios/internal-axios-instance';
...
describe('Connector', () => {
  ...
  const axiosSpy = (method: any, callback: () => void) => {
    return jest
      .spyOn(internalAxiosInstance, method)
  ...
  describe('POST', () => {
    it('should send the request', () => {
      ...
      return connector
        .post('fake', { name: 'unit-test' }, { headers: { name: 'unit-test' } })
        .then((response) => {
          expect(response).toEqual({ data: message });
          expect(spy).toHaveBeenCalled();
        });
  ...
  describe('PUT', () => {
    it('should send the request', () => {
      ...
      return connector
        .put('fake', { name: 'unit-test' }, { headers: { name: 'unit-test' } })
        .then((response) => {
          expect(response).toEqual({ data: message });
          expect(spy).toHaveBeenCalled();
        });
    ...
    it('should get an error', () => {
      ...
      return connector.put('fake', {}).catch((error: any) => {
        expect(error).toBeInstanceOf(ConnectorError);
        expect(spy).toHaveBeenCalled();
      });
    });
  ...
  describe('DELETE', () => {
    it('should send the request', () => {
      ...
      return connector
        .delete('fake', { headers: { name: 'unit-test' } })
        .then((response) => {
          expect(response).toEqual({ data: message });
          expect(spy).toHaveBeenCalled();
        });
